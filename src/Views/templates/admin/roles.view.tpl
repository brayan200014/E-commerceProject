  ...
  <div class="table-responsive">
    <table class="table">
      ...
      <tbody>
        {{foreach items}}
          <tr>
            <td class="text-center align-middle">{{rolescod}}</td>
            <td class="text-center align-middle"><a href="index.php?page=admin_rol&mode=DSP&rolescod={{rolescod}}">{{rolesdsc}}</a></td>
            <td class="text-center align-middle">{{rolesest}}</td>
...
<script>
  document.addEventListener("DOMContentLoaded", function () {
     document.getElementById("btnAdd").addEventListener("click", function (e) {
       e.preventDefault();
       e.stopPropagation();
       window.location.assign("index.php?page=admin_rol&mode=INS&rolescod=0");
     });
   });
</script>